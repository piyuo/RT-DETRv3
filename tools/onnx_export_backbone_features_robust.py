...
Usage:
    python tools/onnx_export_backbone_features_robust.py --input output/rtdetrv3_r18vd_6x_raw.onnx --output output/rtdetrv3_r18vd_6x_backbone.onnx
"""
...
def analyze_model_outputs(model: onnx.ModelProto, input_size: int = 640) -> List[Dict]:
    ...
    print(f"\\n📊 Found {len(candidates)} potential feature map candidates:")
...
def export_backbone_features(input_path: str, output_path: str,
    ...
    if feature_map_name:
        ...
        print(f"\\n🎯 Auto-selected feature map: {selected_candidate['name']}")

    # Validate selection
    is_valid, warnings = validate_feature_map_selection(selected_candidate, input_size)

    print(f"\\n🔍 Validating selected feature map: {selected_candidate['name']}")
    ...
    if warnings and not feature_map_name:
        print("\\n❓ Warnings detected. Consider using a different feature map or --feature-map-name to specify explicitly.")
    ...
    print(f"\\n🔧 Adding '{selected_candidate['name']}' as model output...")
    ...
    print(f"\\n🎉 Backbone feature export completed successfully!")
...
def main():
    ...
    try:
        if args.list_only:
            ...
            if candidates:
                print(f"\\n📋 Use --feature-map-name with one of these candidates:")
                for candidate in candidates:
            ...
            print("\\n📝 Next steps:")
            print("   1. Use the exported model with reid_embeddings_robust.py")
