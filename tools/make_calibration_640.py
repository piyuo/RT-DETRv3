...
# --- Config ---
TARGET_SIZE = 640
# Common normalization for COCO/RT-DETR (if different, adjust here)
...
def preprocess_image(img_path, save_path):
    ...
    # Pad to 640x640 (centered top-left padding)
    padded = np.full((TARGET_SIZE, TARGET_SIZE, 3), 114, dtype=np.uint8)  # 114 = common padding value
    ...
    # Save normalized image as .npy (safe for calibration)
    np.save(save_path, img_norm)
